...
import { movement12Data } from "./movement12Texts";
import { movement13Data, getMovement13Text } from "./movement13Texts";
import { movement14Data } from "./movement14Texts";
import { movement15Data } from "./movement15Texts";
import { movement16Data } from "./movement16Texts";
...
export const getContainerTexts = (
  ...
  // Movement 12
  if (movementId === 12) {
    const texts = movement12Data[language]?.[tonalityId]?.containers?.[containerIndex - 1];
    if (texts) {
      return texts;
    }
  }
  ...
  if (movementId === 13) {
    const tonalityKey = tonalityKeyMap[tonalityId] || "C";
    const containerData = getMovement13Text(language, tonalityKey, String(containerIndex));
    if (containerData) {
      return {
  ...
  // Movement 14
  if (movementId === 14) {
    const texts = movement14Data[language]?.[tonalityId]?.containers?.[containerIndex - 1];
    if (texts) {
      return texts;
    }
  }
  ...
  // Movement 15
  if (movementId === 15) {
    const texts = movement15Data[language]?.[tonalityId]?.containers?.[containerIndex - 1];
    if (texts) {
      return texts;
    }
  }
  ...
  // Movement 16
  if (movementId === 16) {
    const texts = movement16Data[language]?.[tonalityId]?.containers?.[containerIndex - 1];
    if (texts) {
      return texts;
    }
  }
  ...
  // Movement 17
  if (movementId === 17) {
    const texts = movement17Data[language]?.[tonalityId]?.containers?.[containerIndex - 1];
    if (texts) {
      return texts;
    }
  }
  ...
  if (movementId === 18) {
    const tonalityKey = tonalityKeyMap[tonalityId] || "C";
    const mov18Texts = movement18Data.getTexts(tonalityKey);
    const containerData = mov18Texts[language]?.containers?.[containerIndex - 1];
    if (containerData) {
      return {
        label: containerData.redTitle,
        description: containerData.grayText,
  ...
  if (movementId === 19) {
    const tonalityKey = tonalityKeyMap[tonalityId] || "C";
    const mov19Texts = movement19Data.getTexts(tonalityKey, language);
    const containerData = mov19Texts[containerIndex - 1];
    if (containerData) {
      return {
        label: containerData.redTitle,
        description: containerData.grayText,
